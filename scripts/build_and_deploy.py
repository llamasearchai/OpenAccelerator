...
class BuildDeployManager:
        ...
        for command, description in checks:
            try:
                result = self.run_command(
                    f"which {command}", f"Check {description}", check=False
                )
    ...
    def run_medical_compliance_checks(self) -> bool:
        ...
        compliance_scripts = [
            (
                "python scripts/medical_compliance_check.py --output-format json --output-file medical_compliance.json",
                "Medical compliance check",
            ),
            (
                "python scripts/hipaa_compliance_check.py --output-format json --output-file hipaa_compliance.json",
                "HIPAA compliance check",
            ),
            (
                "python scripts/fda_validation_check.py --output-format json --output-file fda_validation.json",
    ...
    def build_documentation(self) -> bool:
        ...
        try:
            ...
            if docs_dir.exists():
                self.run_command(
                    "cd docs && make clean && make html", "Documentation building"
                )
    ...
    def generate_build_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "artifacts": {
                "package": list((self.project_root / "dist").glob("*"))
                if (self.project_root / "dist").exists()
                else [],
                "documentation": (
                    self.project_root / "docs" / "_build" / "html"
                ).exists(),
    ...
    def full_build_pipeline(
        self, skip_tests: bool = False, skip_docs: bool = False
    ) -> bool:
        ...
        if failed_steps:
            self.log(
                f"✗ Build pipeline completed with {len(failed_steps)} failures", "ERROR"
            )
...
def main():
    ...
    parser.add_argument(
        "--deploy-test", action="store_true", help="Deploy to Test PyPI"
    )
