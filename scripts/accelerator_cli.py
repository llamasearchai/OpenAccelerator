...
class OpenAcceleratorCLI:
    ...
    def run_simulation(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            response = self.session.post(f"{self.base_url}/simulation/run", json=config, timeout=30)
            if response.status_code == 200:
                return response.json()
            else:
    ...
    def get_simulation_status(self, simulation_id: str) -> Dict[str, Any]:
        ...
        try:
            response = self.session.get(f"{self.base_url}/simulation/status/{simulation_id}", timeout=5)
            if response.status_code == 200:
                return response.json()
            else:
                return {"error": f"Server returned {response.status_code}"}
    ...
    def run_local_simulation(self, workload_type: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            accel_config = AcceleratorConfig(
                name=kwargs.get("name", "cli_accelerator")
            )
            ...
            print(f"[CONFIG] Array size: {accel_config.array.rows}x{accel_config.array.cols}")
            ...
            if workload_type == "gemm":
                ...
                print(f"[WORKLOAD] GEMM: {workload_config.M}x{workload_config.K} @ {workload_config.K}x{workload_config.P}")
                ...
                workload = GEMMWorkload(GEMMWorkloadConfig(M=4, K=4, P=4), "medical_simulation")
                workload.prepare()
                
                print(f"[WORKLOAD] Medical: {kwargs.get('modality', 'ct_scan')} {kwargs.get('task', 'segmentation')}")
            ...
            print(f"[METRICS] Throughput: {metrics.get('throughput_ops_per_second', 0):.2f} ops/s")
    ...
    if "simulations" in result:
        ...
        if not sims:
            ...
            for i, sim in enumerate(sims, 1):
                print(f"{i}. {sim.get('name', 'Unknown')} - {sim.get('status', 'Unknown')}")
    else:
...
def cmd_benchmark(args, cli: OpenAcceleratorCLI):
    ...
    benchmarks = [
        ...
        {"workload_type": "medical", "modality": "ct_scan", "task": "segmentation", "name": "Medical CT"},
    ]
    ...
    for benchmark in benchmarks:
        ...
        if result.get("success"):
            print(f"[SUCCESS] {benchmark['name']}: {result.get('simulation_time', 0):.3f}s")
        else:
...
def cmd_medical(args, cli: OpenAcceleratorCLI):
    ...
    if result.get("success"):
        ...
        print(f"[ERROR] Medical compliance check failed: {result.get('error', 'Unknown error')}")
...
Author: Nik Jois <nikjois@llamasearch.ai>
    ...
    parser.add_argument("--version", action="version", version="OpenAccelerator CLI 1.0.0")
    ...
    sim_parser.add_argument("workload_type", choices=["gemm", "medical"], 
    ...
    sim_parser.add_argument("--local", action="store_true", 
                           help="Run locally instead of via API")
    ...
    sim_parser.add_argument("--modality", default="ct_scan", 
                           choices=["ct_scan", "mri", "xray", "ultrasound"],
                           help="Medical imaging modality")
    sim_parser.add_argument("--task", default="segmentation",
                           choices=["segmentation", "classification", "detection"],
                           help="Medical AI task")
    sim_parser.add_argument("--image-size", nargs=3, type=int, default=[256, 256, 16],
    ...
    # Medical command
    medical_parser = subparsers.add_parser("medical", help="Run medical compliance check")
