...
def analyze_security_posture(artifacts: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "medical_compliance" in artifacts:
        medical_data = artifacts["medical_compliance"].get(
            "medical_compliance_report.json", {}
        )
    ...
    analysis["compliance_status"]["medical"] = (
...
def generate_html_report(
    artifacts: Dict[str, Any], analysis: Dict[str, Any], output_dir: Path
):
...
"""
    ...
    for category, data in artifacts.items():
        status = (
            "Completed"
            if any(isinstance(v, dict) and "error" not in v for v in data.values())
            else "Partial"
        )
        file_count = len(
            [f for f in data.values() if isinstance(f, dict) and "error" not in f]
        )
        html_template += f"""
            <tr>
    ...
    report = {
        ...
        "summary": {
            ...
            "excellent_categories": len(
                [s for s in analysis["category_scores"].values() if s >= 90]
            ),
            "good_categories": len(
                [s for s in analysis["category_scores"].values() if 70 <= s < 90]
            ),
            "poor_categories": len(
                [s for s in analysis["category_scores"].values() if s < 70]
            ),
        },
    }
...
def main():
    ...
    parser.add_argument(
        "--artifacts-dir", required=True, help="Directory containing security artifacts"
    )
    parser.add_argument(
