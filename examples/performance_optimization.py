...
class PerformanceOptimizer:
    ...
    def create_accelerator_variants(self) -> List[AcceleratorConfig]:
        ...
        medical_config = AcceleratorConfig(
            ...
            medical=MedicalConfig(
                enable_medical_mode=True, fda_validation=True, phi_compliance=True
            ),
        )
    ...
    def benchmark_configuration(
        self, accel_config: AcceleratorConfig, workloads: List[Tuple[Any, str]]
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                if hasattr(workload, "M"):  # GEMM workload
                    ...
                    theoretical_cycles = max(
                        ...
                        ops
                        // (
                            accel_config.array.rows * accel_config.array.cols
                        ),  # Compute bound
                    )
                    actual_cycles = theoretical_cycles + np.random.randint(
                        0, 5
                    )  # Add some variance
                    ...
                    efficiency = ops / (
                        actual_cycles
                        * accel_config.array.rows
                        * accel_config.array.cols
                    )
                    ...
                    theoretical_cycles = ops // (
                        accel_config.array.rows * accel_config.array.cols * 10
                    )
                ...
                print(
                    f"    [SUCCESS] {throughput:.2f} GOPs, {efficiency:.1%} efficiency"
                )
        ...
        if successful_results:
            results["summary"] = {
                ...
                "avg_throughput_gops": np.mean(
                    [r["throughput_gops"] for r in successful_results]
                ),
                "avg_efficiency": np.mean(
                    [r["efficiency"] for r in successful_results]
                ),
                "total_execution_time_s": total_time,
                "peak_throughput_gops": max(
                    [r["throughput_gops"] for r in successful_results]
                ),
    ...
    def analyze_results(self, all_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for result in valid_results:
            score = (
                result["summary"]["avg_throughput_gops"]
                * result["summary"]["avg_efficiency"]
                * result["summary"]["success_rate"]
            )
            if (
                not analysis["best_overall"]
                or score > analysis["best_overall"]["score"]
            ):
                analysis["best_overall"] = {**result, "score": score}

        # Best throughput
        best_throughput = max(
            valid_results, key=lambda x: x["summary"]["avg_throughput_gops"]
        )
        analysis["best_throughput"] = best_throughput

        # Best efficiency
        best_efficiency = max(
            valid_results, key=lambda x: x["summary"]["avg_efficiency"]
        )
        ...
        if medical_results:
            analysis["best_medical"] = max(
                medical_results, key=lambda x: x["summary"]["avg_throughput_gops"]
            )
    ...
    def generate_recommendations(self, results: List[Dict[str, Any]]) -> List[str]:
        ...
        array_sizes = [
            (r["config_name"], int(r["array_size"].split("x")[0])) for r in results
        ]
        if len(set(size for _, size in array_sizes)) > 1:
            best_config = max(
                results, key=lambda x: x["summary"]["avg_throughput_gops"]
            )
            recommendations.append(
        ...
        if medical_results:
            ...
            general_perf = max(
                r["summary"]["avg_throughput_gops"]
                for r in results
                if "Medical" not in r["config_name"]
            )
    ...
    def create_visualizations(
        self, all_results: List[Dict[str, Any]], analysis: Dict[str, Any]
    ):
        ...
        fig.suptitle(
            "OpenAccelerator Performance Analysis", fontsize=16, fontweight="bold"
        )
        ...
        bars1 = ax1.bar(
            config_names, throughputs, color=["#FF6B6B", "#4ECDC4", "#45B7D1"]
        )
        ...
        bars2 = ax2.bar(
            config_names, efficiencies, color=["#96CEB4", "#FFEAA7", "#DDA0DD"]
        )
        ...
        bars3 = ax3.bar(
            config_names, success_rates, color=["#A8E6CF", "#FFB3BA", "#FFDFBA"]
        )
        ...
        print("[METRICS] Performance charts saved to performance_analysis.png")
    ...
    def save_detailed_report(
        ...
        report = {
            ...
            "summary": {
                ...
                "successful_configurations": len(
                    [r for r in all_results if r.get("summary")]
                ),
                "best_overall_config": analysis.get("best_overall", {}).get(
                    "config_name"
                ),
                "best_throughput_config": analysis.get("best_throughput", {}).get(
                    "config_name"
                ),
                "best_efficiency_config": analysis.get("best_efficiency", {}).get(
                    "config_name"
                ),
            },
        ...
        print("[REPORT] Detailed report saved to performance_report.json")
    ...
    def run_complete_analysis(self):
        ...
        if analysis["best_overall"]:
            ...
            print(
                f"   Avg Throughput: {best['summary']['avg_throughput_gops']:.2f} GOPs"
            )
        ...
        if analysis["best_throughput"]:
            ...
            print(
                f"[SYSTEM] Best Throughput: {best_tp['config_name']} ({best_tp['summary']['avg_throughput_gops']:.2f} GOPs)"
            )

        if analysis["best_efficiency"]:
            best_eff = analysis["best_efficiency"]
            print(
                f"⚡ Best Efficiency: {best_eff['config_name']} ({best_eff['summary']['avg_efficiency']:.1%})"
            )
        ...
        print("[METRICS] Results: performance_analysis.png")
        print("[REPORT] Report: performance_report.json")
