...
def test_systolic_array():
    ...
    try:
        ...
        config = AcceleratorConfig(array=ArrayConfig(rows=2, cols=2, dataflow="OS"))
...
def test_api_server():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.info(
                f"[SUCCESS] API server is healthy: {health_data.get('status', 'unknown')}"
            )
            return True
        else:
            logger.warning(
                f"[WARNING]  API server responded with status {response.status_code}"
            )
            return False
