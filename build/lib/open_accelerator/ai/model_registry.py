...
class ModelRegistry:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self._model_instances: Dict[str, OpenAI] = {}
    ...
    def get_dspy_model(self, model_name: str, **kwargs) -> OpenAI:
        """Get a DSPy-wrapped OpenAI model instance."""
        if model_name not in self._model_instances:
            model_config = self.get_model(model_name)
            if not model_config:
                raise ValueError(f"Model {model_name} not found in registry")

            # Create DSPy OpenAI instance
            self._model_instances[model_name] = OpenAI(
                model=model_config.api_name,
                api_key=self.api_key,
                max_tokens=min(
                    model_config.capabilities.max_tokens, kwargs.get("max_tokens", 4096)
                ),
                **kwargs,
            )
    ...
    def estimate_cost(
        ...
        """Estimate cost for a model call (placeholder - would need real pricing data)."""
        model = self.get_model(model_name)
        if not model:
            return 0.0

        # Placeholder pricing (would need to be updated with actual OpenAI pricing)
        base_rates = {
