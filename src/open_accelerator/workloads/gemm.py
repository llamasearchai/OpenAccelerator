...
class GEMMWorkload(BaseWorkload):
    ...
    def __init__(self, config: GEMMWorkloadConfig, name: str = "GEMM"):
        ...
        self.requirements = WorkloadRequirements(
            ...
            supported_data_types=["float32", "int32"] if config.use_integers else ["float32"],
        )
        ...
        logger.info(f"Initialized GEMM workload: {config.M}x{config.K} @ {config.K}x{config.P}")
    ...
    def get_complexity(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_accesses": self.config.M * self.config.K + self.config.K * self.config.P + self.config.M * self.config.P,
            "arithmetic_intensity": (2 * self.config.M * self.config.K * self.config.P) / (self.config.M * self.config.K + self.config.K * self.config.P + self.config.M * self.config.P),
            "matrix_dimensions": {
    ...
    def prepare(self, **kwargs) -> None:
        ...
        if self.config.use_integers:
            self.matrix_A = np.random.randint(
                int(data_range[0]), int(data_range[1]) + 1,
        ...
        # Update metrics
        self.metrics.total_operations = 2 * self.config.M * self.config.K * self.config.P  # MACs
        
        self.is_prepared = True
        logger.info(f"GEMM workload prepared with {self.metrics.total_operations} operations")
    ...
    def validate_output(self, actual_output: Dict[str, np.ndarray]) -> bool:
        ...
        if actual_C.shape != expected_C.shape:
            logger.error(f"Shape mismatch: expected {expected_C.shape}, got {actual_C.shape}")
            return False
    ...
    def get_operations(self) -> List[Dict[str, Any]]:
        ...
        assert self.matrix_A is not None and self.matrix_B is not None and self.expected_C is not None
    ...
    def get_description(self) -> str:
        ...
        sparsity_str = f", {self.config.sparsity:.1%} sparse" if self.config.sparsity > 0 else ""
...
def create_rectangular_gemm(M: int, K: int, P: int, name: str = "RectangularGEMM") -> GEMMWorkload:
