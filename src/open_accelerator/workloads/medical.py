...
from ..utils.config import WorkloadConfig, WorkloadType
...
class MedicalConvolution(BaseWorkload):
    ...
    def prepare(self, **kwargs) -> None:
        """Prepare the medical convolution workload for execution."""
...
class MedicalResNet(BaseWorkload):
    ...
    def prepare(self, **kwargs) -> None:
        """Prepare the medical ResNet workload for execution."""
        if not self.is_prepared:
...
class MedicalTransformer(BaseWorkload):
    ...
    def prepare(self, **kwargs) -> None:
        """Prepare the medical Transformer workload for execution."""
        if not self.is_prepared:
    ...
    def generate_attention_gemm_ops(self) -> list[tuple[np.ndarray, np.ndarray]]:
        ...
        for layer in range(self.num_layers):
            ...
            for head in range(self.num_heads):
                ...
                attention_scores = np.random.randn(
...
MedicalImagingWorkload = MedicalConvolution
