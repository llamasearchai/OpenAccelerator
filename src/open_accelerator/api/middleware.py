...
class MedicalComplianceMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self.enable_audit_logging and request.url.path.startswith(
            "/api/v1/medical/"
        ):
            self.audit_logger.info(
...
def get_health_metrics() -> Dict[str, Any]:
    ...
    error_rate = app_health_metrics["error_count"] / max(
        app_health_metrics["request_count"], 1
    )
...
def setup_middleware(app: FastAPI, config: Optional[Dict[str, Any]] = None) -> None:
    ...
    # Rate limiting middleware
    app.add_middleware(
        RateLimitMiddleware, requests_per_minute=config.get("rate_limit", 100)
    )

    # Request logging middleware
    app.add_middleware(
        LoggingMiddleware, log_body=config.get("log_request_body", False)
    )
