...
from .models import (
    SimulationRequest, SimulationResponse, SimulationResult, SimulationStatus,
    AgentRequest, AgentResponse, AgentType,
    MedicalWorkflowRequest, MedicalWorkflowResponse,
    HealthResponse, HealthStatus,
    SimulationListResponse, MetricsResponse,
    ErrorResponse
)
...
async def _run_simulation_async(
    ...
    try:
        ...
        result = orchestrator.run_single_simulation(
            accel_config,
            workload,
            simulation_name=f"api_simulation_{simulation_id}"
        )
        ...
        simulation_result = SimulationResult(
            ...
            execution_time_seconds=time.time() - active_simulations[simulation_id]["start_time"],
...
def _convert_api_config_to_internal(api_config) -> AcceleratorConfigUtil:
    ...
    from ..utils.config import (
        ArrayConfig, MemoryHierarchyConfig, PowerConfig, MedicalConfig,
        BufferConfig, DataType, AcceleratorType, DataflowType
    )
    ...
    accel_type = AcceleratorType.MEDICAL if api_config.medical_mode else AcceleratorType.BALANCED
