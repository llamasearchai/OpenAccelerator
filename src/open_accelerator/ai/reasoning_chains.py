...
class ReasoningChain(ABC):
    ...
    @abstractmethod
    async def process(self, prompt: str, context: Dict[str, Any] = None) -> ReasoningResult:
    ...
    def _calculate_quality_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "overall_quality": (avg_confidence + consistency_score + completeness_score) / 3
        }
...
class ChainOfThought(ReasoningChain):
    ...
    async def process(self, prompt: str, context: Dict[str, Any] = None) -> ReasoningResult:
        ...
        confidence_score = sum(step.confidence for step in self.reasoning_steps) / len(self.reasoning_steps)
    ...
    async def process(self, prompt: str, context: Dict[str, Any] = None) -> ReasoningResult:
        ...
        for step in self.reasoning_steps:
            if step.step_type == "reflection":
                ...
                if "improve" in step.response.lower() or "better" in step.response.lower():
    ...
    async def process(self, prompt: str, context: Dict[str, Any] = None) -> ReasoningResult:
        ...
        for i, step_config in enumerate(self.custom_steps):
            step_prompt = step_config["prompt"].format(
                ...
                previous_steps="\n".join([step.response for step in self.reasoning_steps])
            )
