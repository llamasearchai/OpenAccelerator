...
from open_accelerator.medical.compliance import (
    HIPAACompliance, FDACompliance, MedicalComplianceManager,
    ComplianceLevel, ComplianceStatus, AuditEvent
)
from open_accelerator.medical.imaging import (
    MedicalImageProcessor, ImageModality, ImageFormat,
    DicomProcessor, NiftiProcessor, create_medical_image_processor
)
from open_accelerator.medical.models import (
    MedicalModel, ModelType, ModelMetrics, ModelValidation,
    create_segmentation_model, create_classification_model, create_detection_model
)
from open_accelerator.medical.optimization import (
    MedicalOptimizer, OptimizationStrategy, OptimizationMetrics,
    create_medical_optimizer, optimize_for_medical_workload
)
from open_accelerator.medical.validation import (
    MedicalValidator, ValidationLevel, ValidationResult,
    create_medical_validator, validate_medical_model
)
from open_accelerator.medical.workflows import (
    MedicalWorkflow, WorkflowStep, WorkflowStatus,
    create_medical_workflow, execute_medical_workflow
)
...
class TestHIPAACompliance:
    ...
    def test_phi_anonymization(self, hipaa_compliance):
        ...
        assert "patient_name" not in anonymized_data or anonymized_data["patient_name"] == "[REDACTED]"
...
class TestMedicalImageProcessor:
    ...
    def test_medical_specific_processing(self, image_processor):
        ...
        # Test windowing
        windowed = image_processor.apply_window(ct_image, window_center=40, window_width=400)
    ...
    def test_privacy_preservation(self, image_processor):
        ...
        cleaned_image, cleaned_metadata = image_processor.remove_phi_metadata(test_image, metadata)
...
class TestMedicalOptimizer:
    ...
    def test_memory_optimization(self, medical_optimizer):
        ...
        assert optimization_result.optimized_profile["model_size"] <= memory_profile["model_size"]
    ...
    def test_medical_specific_optimization(self, medical_optimizer):
        ...
        optimization_result = medical_optimizer.optimize_for_medical_workload(medical_workload)
...
class TestMedicalValidator:
    ...
    def test_clinical_validation(self, medical_validator):
        ...
        # Mock clinical data
        clinical_data = {
            "patient_demographics": {"age_range": "18-80", "gender_distribution": "balanced"},
    ...
    def test_regulatory_validation(self, medical_validator):
        ...
        validation_result = medical_validator.validate_regulatory_compliance(regulatory_data)
...
class TestMedicalWorkflow:
    ...
    def test_workflow_validation(self, medical_workflow):
        ...
        valid_steps = [
            ...
            WorkflowStep("output_validation", "validate_output", {"check_quality": True})
        ]
    ...
    def test_workflow_monitoring(self, medical_workflow):
        ...
        with patch.object(medical_workflow, '_execute_step') as mock_execute:
            mock_execute.return_value = {"status": "success", "metrics": {"duration": 0.5}}
...
class TestMedicalIntegration:
    ...
    def test_end_to_end_medical_workflow(self, medical_config):
        ...
        steps = [
            ...
            WorkflowStep("segmentation", "segment_organs", {"model": "organ_segmentation"}),
            WorkflowStep("validation", "validate_results", {"clinical_review": True}),
            WorkflowStep("compliance_check", "check_compliance", {"hipaa": True, "fda": True}),
            WorkflowStep("audit_log", "log_audit_event", {"event_type": "analysis_complete"})
        ]
