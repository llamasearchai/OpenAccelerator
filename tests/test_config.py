...
class TestArrayConfig:
    ...
    def test_array_config_custom_values(self):
        ...
        config = ArrayConfig(rows=32, cols=32, frequency=2e9, voltage=1.5)
...
class TestWorkloadConfig:
    ...
    def test_workload_config_defaults(self):
        ...
        config = WorkloadConfig(
            workload_type=WorkloadType.GEMM, gemm_M=8, gemm_K=8, gemm_P=8
        )
    ...
    def test_gemm_workload_config(self):
        ...
        config = WorkloadConfig(
            workload_type=WorkloadType.GEMM, gemm_M=64, gemm_K=32, gemm_P=16
        )
    ...
    def test_workload_config_validation(self):
        ...
        with pytest.raises(
            ValueError, match="GEMM workload requires M, K, P dimensions"
        ):
        ...
        with pytest.raises(ValueError, match="GEMM dimensions must be positive"):
            WorkloadConfig(
                workload_type=WorkloadType.GEMM, gemm_M=-1, gemm_K=32, gemm_P=16
            )

        # Test convolution validation
        with pytest.raises(
            ValueError,
            match="Convolution workload requires all input/output dimensions",
        ):
            WorkloadConfig(
                workload_type=WorkloadType.CONVOLUTION, conv_input_height=224
            )

        # Test medical imaging validation
        with pytest.raises(
            ValueError, match="Medical imaging workload requires modality"
        ):
