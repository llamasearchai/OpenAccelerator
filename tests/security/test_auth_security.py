...
class AuthSecurityTester:
        ...
        for token in test_tokens:
            try:
                ...
                test_result = {
                    ...
                    "token": token,
                    "details": response.json() if response.status_code not in [401, 403] else None
                }
        ...
        for test_case in test_cases:
            try:
                ...
                test_result = {
                    ...
                    "description": test_case['description'],
                    "details": response.json() if not ((response.status_code == 200) == expected_success) else None
                }
        ...
        for test_case in test_cases:
            try:
                params = {
                    ...
                    "redirect_uri": test_case.get("redirect_uri", "https://app.example.com/callback"),
                    "scope": "read write"
                }
                ...
                test_result = {
                    ...
                    "expected": "400 or 401" if test_case["should_fail"] else "200 or 302",
                    "actual": response.status_code,
                    "passed": (response.status_code in [400, 401]) == test_case["should_fail"],
                    "description": test_case['description'],
                    "params": params,
                    "details": response.json() if response.status_code not in [200, 302, 400, 401] else None
                }
                ...
                self.results["oauth_tests"].append({
                    "test": f"OAuth authorization: {test_case['description']}",
                    "error": str(e),
                    "passed": test_case["should_fail"]  # Exception is good for invalid requests
                })
        ...
        for test_case in test_cases:
            try:
                ...
                test_result = {
                    ...
                    "passed": (response.status_code in [400, 401]) == test_case["should_fail"],
                    "description": test_case['description'],
                    "details": response.json() if response.status_code not in [200, 400, 401] else None
                }
    ...
    def _test_session_fixation(self):
        ...
        try:
            ...
            test_result = {
                ...
                "details": response.json() if response.status_code not in [200, 302] else None
            }
        ...
        for test_case in test_cases:
            try:
                ...
                test_result = {
                    ...
                    "actual": response.status_code,
                    "passed": (response.status_code == 200) == test_case["should_be_valid"],
                    "description": test_case['description'],
                    "session_age": test_case["age_seconds"],
                    "details": response.json() if not ((response.status_code == 200) == test_case["should_be_valid"]) else None
                }
    ...
    def _test_token_revocation(self):
        ...
        try:
            ...
            test_result = {
                ...
                "details": response.json() if response.status_code not in [200, 204] else None
            }
            ...
            try:
                ...
                for endpoint, method, required_scope in test_endpoints:
                    ...
                    test_result = {
                        ...
                        "endpoint": endpoint,
                        "details": response.json() if not ((200 <= response.status_code < 300) == should_succeed) else None
                    }
    ...
    def _create_invalid_audience_jwt_token(self) -> Dict[str, str]:
        ...
        payload = {"sub": "test", "aud": "wrong-audience", "exp": int(time.time()) + 3600}
        token = self._create_test_jwt_token(payload)
    ...
    def _manipulate_jwt_token(self, token: str, manipulation: Dict[str, Any]) -> str:
        ...
        if manipulation.get("header"):
            header_b64 = base64.urlsafe_b64encode(
                json.dumps(manipulation["header"]).encode()
            ).decode().rstrip('=')
            parts[0] = header_b64
        ...
        if manipulation.get("payload"):
            payload_b64 = base64.urlsafe_b64encode(
                json.dumps(manipulation["payload"]).encode()
            ).decode().rstrip('=')
            parts[1] = payload_b64
    ...
    def _generate_summary(self):
        ...
        logger.info(f"Authentication security tests completed: {passed_tests}/{total_tests} passed ({self.results['summary']['overall']['pass_rate']:.1f}%)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Authentication Security Testing Suite")
    parser.add_argument("--target-url", default="http://localhost:8000", help="Target API URL")
    parser.add_argument("--output-file", default="auth_security_report.json", help="Output file for results")
