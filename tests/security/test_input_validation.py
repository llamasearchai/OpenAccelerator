...
class InputValidationTester:
    ...
    def _test_simulation_input_validation(self):
        ...
        test_cases = [
            ...
            {
                "payload": {
                    "accelerator_config": "not_an_object",
                    "workload": 123
                },
        ...
        for test_case in test_cases:
            try:
                ...
                self.results["validation_tests"].append({
                    "test": f"Simulation validation: {test_case['description']}",
                    "error": str(e),
                    "passed": test_case["should_fail"]  # Exception is good for invalid input
                })
            ...
            for payload in sql_payloads:
                try:
                    ...
                    test_result = {
                        ...
                        "field": field,
                        "payload": payload,
                        "details": response.json() if not (400 <= response.status_code < 500) else None
                    }
    ...
    def _test_format_validation(self):
        ...
        format_tests = [
            # Email format
            {
                ...
                "invalid_values": ["invalid-email", "@domain.com", "user@", "user@domain"]
            },
            ...
            # UUID format
            {
                ...
                "invalid_values": ["not-a-uuid", "123-456-789", "123e4567-e89b-12d3-a456"]
            },
            ...
            for valid_value in format_test["valid_values"]:
                try:
                    ...
                    test_result = {
                        ...
                        "value": valid_value,
                        "valid": True,
                        "details": response.json() if response.status_code >= 400 else None
                    }
    ...
    def _test_file_upload_validation(self):
        ...
        for file_test in file_tests:
            try:
                ...
                files = {
                    "file": (file_test["filename"], file_test["content"], file_test["content_type"])
                }
                ...
                self.results["file_upload_tests"].append({
                    "test": f"File upload: {file_test['filename']}",
                    "error": str(e),
                    "passed": not file_test["should_pass"]  # Exception is good for invalid uploads
                })
    ...
    def _generate_summary(self):
        ...
        logger.info(f"Input validation security tests completed: {passed_tests}/{total_tests} passed ({self.results['summary']['overall']['pass_rate']:.1f}%)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Input Validation Security Testing Suite")
    parser.add_argument("--target-url", default="http://localhost:8000", help="Target API URL")
    parser.add_argument("--output-file", default="input_validation_report.json", help="Output file for results")
