...
class SystemHealthMonitor:
    ...
    def check_package_imports(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # AI imports
            from open_accelerator.ai.agents import AgentType, AgentOrchestrator
            ...
            from open_accelerator.medical.workflows import MedicalWorkflow
    ...
    def run_health_check(self) -> Dict[str, Any]:
        ...
        for component, check_func in component_checks.items():
            try:
                ...
                if result["status"] == "HEALTHY":
                    ...
                    error_count += 1
                    logger.error(f"❌ {component}: ERROR - {result.get('error', 'Unknown error')}")
        ...
        overall_status = "HEALTHY" if error_count == 0 else "DEGRADED" if success_rate >= 70 else "CRITICAL"
        ...
        logger.info(f"Health check completed: {overall_status} ({success_rate:.1f}% success rate)")
        ...
        while True:
            try:
                ...
                if self.health_status['overall_status'] == 'CRITICAL':
                    logger.critical("CRITICAL SYSTEM STATUS - Immediate attention required!")
                elif self.health_status['overall_status'] == 'DEGRADED':
                    logger.warning("DEGRADED SYSTEM STATUS - Some components need attention")
                else:
